...
public class BookAgeCoeff extends Configured implements Tool {

	
   public static void main( String[] args) throws  Exception {
      int res  = ToolRunner .run( new BookAgeCoeff(), args);
      System .exit(res);
   }
...
//Mapper output key is age and value is the book id and rating given by each user in that age
...
String country=agecountry[1].trim();
...
//reduce output key is age and value is all the book id and corresponding rating given by each user in that age
...
//Mapper output key is age and combination of all pair of books and the average of their ratings given in that age
   ...
   public static class Map2 extends Mapper<LongWritable ,  Text ,  Text ,  Text > {
	      ...
	      public void map( LongWritable offset,  Text lineText,  Context context)
	        throws  IOException,  InterruptedException {
	    	  ...
	    	  String[] line  = lineFull[1].toString().trim().split("######");
	    	  int len_arr = 0;
...
//reducer calculates correlation score based on coefficients for each pair of books
...
{			   	  System.out.println("Inside Reduce2 ---->>>jaccard_coeff, corr, cos"+jaccard_coeff+" "+corr+" "+cosine);
	    		  context.write(new Text(movieId[0]+","+movieId[1]+","+movieId[2]), new Text(deciFormat.format(corr)));
	    		  context.write(new Text(movieId[0]+","+movieId[2]+","+movieId[1 ]), new Text(deciFormat.format(corr)));
}
			   }
   ...
   // Following MR is used for sorting based on the scores
